...
const PerformanceDashboard: React.FC<PerformanceDashboardProps> = ({
  ...
  const apiStats = Object.keys(apiMetrics)
    .map((api) => getApiStats(api))
    .filter(Boolean);
        ...
        <div className="bg-black bg-opacity-90 text-white p-4 rounded-lg shadow-xl max-w-sm font-mono text-xs">
          ...
          {memoryInfo && (
            <div className="mb-2">
              ...
              <div>
                Used: <span className="text-yellow-400">{memoryInfo.used}</span>
              </div>
              <div>
                Total: <span className="text-gray-400">{memoryInfo.total}</span>
              </div>
              <div>
                Limit: <span className="text-gray-400">{memoryInfo.limit}</span>
              </div>
            ...
            <div className="mb-2">
              ...
              <div>
                ...
                <span className="text-yellow-400">
                  {performanceSummary.avgRenderTime}ms
                </span>
              </div>
              <div>
                Max:{" "}
                <span className="text-red-400">
                  {performanceSummary.maxRenderTime}ms
                </span>
              </div>
              <div>
                Count:{" "}
            ...
            <div className="mb-2">
              ...
              {apiStats.slice(0, 3).map((stat, index) => (
                <div key={index} className="text-xs">
                  ...
                  <div>
                    Avg:{" "}
                    <span className="text-yellow-400">{stat?.avgTime}ms</span>
                  </div>
                </div>
              ))}
            </div>
