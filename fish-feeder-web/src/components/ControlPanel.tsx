...
const ControlPanel: React.FC = () => {
  const { controlBlower, controlActuator } = useApi();
  ...
  const handleMotorCommand = async (motor: string, command: string) => {
    ...
    // Log motor command
    logger.buttonPress(
      `MOTOR_${motor.toUpperCase()}_${command}`,
      "ControlPanel",
      {
        motor,
        command,
    ...
    try {
      ...
      if (motor === "b" || motor === "blower") {
        ...
        // Actuator control
        const action =
          command === "1" ? "up" : command === "2" ? "down" : "stop";

        response = await controlActuator(action);
      } else if (motor === "g" || motor === "auger") {
        // Auger control - we'll simulate this since there's no direct auger control
        logger.warn("MOTOR", "AUGER_NOT_IMPLEMENTED", { command });
        response = {
          status: "offline",
          message: "Auger control not implemented via Firebase",
        };
      } else {
      ...
      if (response.status === "success" || response.status === "offline") {
        setLastCommandStatus((prev) =>
          prev ? { ...prev, status: "success" } : null,
        );
        setMotorStatus((prev) => ({
          ...prev,
          [motor]: command === "1" ? "running" : "stopped",
        }));
        ...
        if (command === "1") {
          ...
          logger.warn("MOTOR", "AUTO_TIMEOUT_DISABLED", {
            motor,
            message: "Using event-driven status monitoring",
          });
        }
      } else {
        throw new Error(
          `Command failed: ${response.message || "Unknown error"}`,
        );
      }
    } catch (error) {
      setLastCommandStatus((prev) =>
