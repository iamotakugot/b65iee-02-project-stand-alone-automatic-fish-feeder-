...
export const useApiSensorData = (): UseApiSensorDataReturn => {
  ...
  const fetchSensorData = useCallback(async (): Promise<boolean> => {
    ...
    if (response && response.status === "success" && response.data) {
      ...
      console.log(
        `✅ Sensor data fetched (${response.source}):`,
        response.data,
      );
      setLoading(false);
  ...
  const fetchCachedSensorData = useCallback(async (): Promise<boolean> => {
    ...
    if (response && response.data) {
      ...
      setLastUpdate(new Date().toLocaleTimeString());
      console.log(
        `✅ Cached sensor data fetched (${response.source}):`,
        response.data,
      );
      setLoading(false);
  ...
  const controlFeeder = useCallback(
    async (action: "small" | "medium" | "large" | number): Promise<boolean> => {
  ...
  const setMotorPWM = useCallback(
    async (motorId: string, speed: number): Promise<boolean> => {
      ...
      console.log(
        `Motor PWM ${motorId}:${speed} command:`,
        success ? "✅ Success" : "❌ Failed",
      );
  ...
  const calibrateWeight = useCallback(
    async (knownWeight: number): Promise<boolean> => {
      ...
      console.log(
        `Weight calibration (${knownWeight}kg) command:`,
        success ? "✅ Success" : "❌ Failed",
      );
