...
const API_BASE_URL = isFirebaseHosting()
  ? "firebase-only"
  : "http://localhost:5000";
...
export const useApiConnection = () => {
  ...
  useEffect(() => {
    ...
    if (FIREBASE_ONLY_MODE) {
      console.log(
        "🔥 Running in Firebase-only mode - skipping WebSocket connection",
      );
      setLoading(false);
  ...
  const apiRequest = useCallback(
    async (endpoint: string, options: RequestInit = {}) => {
      ...
      if (FIREBASE_ONLY_MODE) {
        ...
        return {
          status: "firebase-only",
          message: "Running in Firebase-only mode",
        };
      }
