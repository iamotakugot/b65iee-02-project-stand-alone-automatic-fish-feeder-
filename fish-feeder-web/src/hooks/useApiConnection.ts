...
const API_BASE_URL = isFirebaseHosting() ? 'disabled' : 'http://localhost:5000';
const WS_URL = isFirebaseHosting() ? 'disabled' : 'http://localhost:5000';
const OFFLINE_MODE = isFirebaseHosting(); // Use Firebase-only in production to avoid HTTPS/HTTP mixed content
...
export const useApiConnection = () => {
  ...
  useEffect(() => {
    // Skip WebSocket connection in offline mode (Firebase hosting)
    if (OFFLINE_MODE) {
      console.log('🔥 Running in offline mode - skipping WebSocket connection');
      setLoading(false);
  ...
  const apiRequest = useCallback(async (endpoint: string, options: RequestInit = {}) => {
    ...
    if (OFFLINE_MODE) {
      console.log(`🔥 Offline mode: skipping API call to ${endpoint}`);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      // Skip initial API data fetch in offline mode
      if (OFFLINE_MODE) {
        console.log('🔥 Offline mode: using Firebase data only');
        setData({
          sensors: {},
          status: {
            online: false,
  ...
  return {
    ...
    loading,
    error,
    connected: OFFLINE_MODE ? false : (socket?.connected || false),
    // Control functions
    controlLED,
    controlFan,
    controlFeeder,
