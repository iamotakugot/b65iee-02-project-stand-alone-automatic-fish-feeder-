...
export const useOptimizedFirebase = (path?: string) => {
    ...
    async (endpoint: string, options: RequestInit = {}) => {
      ...
      try {
        ...
        if (apiUrl.includes("localhost")) {
          ...
          console.warn(
            '   Then set URL: localStorage.setItem("ngrok_url", "https://abc123.ngrok.io")',
          );
        }
  ...
  useEffect(() => {
    ...
    console.log(
      "🎯 ON-DEMAND MODE: No auto-refresh. Call refresh() manually to update data.",
    );
  }, [fetchData]);
  ...
  const controlLED = useCallback(
    async (action: "on" | "off" | "toggle"): Promise<boolean> => {
      try {
  ...
  const controlFeeder = useCallback(
    async (
      action: "on" | "off" | "small" | "medium" | "large" | "auto",
    ): Promise<boolean> => {
      try {
  ...
  const controlActuator = useCallback(
    async (action: "up" | "down" | "stop"): Promise<boolean> => {
      try {
  ...
  const setMotorPWM = useCallback(
    async (motorId: string, speed: number): Promise<boolean> => {
      try {
  ...
  const calibrateWeight = useCallback(
    async (knownWeight: number): Promise<boolean> => {
      try {
  ...
  const setDeviceTiming = useCallback(
    async (timings: {
      actuatorUp: number;
      actuatorDown: number;
  ...
  const updateDeviceTiming = useCallback(
    async (timing: any) => {
      try {
        return await apiRequest("/api/device/timing", {
          method: "POST",
  ...
  const sendArduinoCommand = useCallback(
  ...
  const directControl = useCallback(
