...
const SensorCharts: React.FC = () => {
      ...
      const generateData = () => {
        ...
        for (let i = points; i >= 0; i--) {
          const timestamp = new Date(
            now.getTime() - (i * (totalMinutes * 60 * 1000)) / points,
          );
          ...
          data.push({
            ...
            batteryVoltage: values.batteryVoltage
              ? Number((values.batteryVoltage * variation()).toFixed(2))
              : null,
            batteryPercentage: values.batteryPercentage
              ? Number((values.batteryPercentage * variation()).toFixed(1))
              : null,
            loadVoltage: values.loadVoltage
              ? Number((values.loadVoltage * variation()).toFixed(2))
              : null,
            loadCurrent: values.loadCurrent
              ? Number((values.loadCurrent * variation()).toFixed(3))
              : null,
            solarVoltage: values.solarVoltage
              ? Number((values.solarVoltage * variation()).toFixed(2))
              : null,
            solarCurrent: values.solarCurrent
              ? Number((values.solarCurrent * variation()).toFixed(3))
              : null,
            feederTemp: values.feederTemp
              ? Number((values.feederTemp * variation()).toFixed(1))
              : null,
            systemTemp: values.systemTemp
              ? Number((values.systemTemp * variation()).toFixed(1))
              : null,
            feederHumidity: values.feederHumidity
              ? Number((values.feederHumidity * variation()).toFixed(1))
              : null,
            systemHumidity: values.systemHumidity
              ? Number((values.systemHumidity * variation()).toFixed(1))
              : null,
            feederWeight: values.feederWeight
              ? Number((values.feederWeight * variation()).toFixed(1))
              : null,
            soilMoisture: values.soilMoisture
              ? Number((values.soilMoisture * variation()).toFixed(1))
              : null,
          });
        }
  ...
  useEffect(() => {
    const category =
      SENSOR_CATEGORIES[selectedCategory as keyof typeof SENSOR_CATEGORIES];
  ...
  const chartData = useMemo(() => {
    return historicalData.filter((record) => {
      return selectedSensors.some(
        (sensor) => record[sensor] !== null && record[sensor] !== undefined,
      );
    });
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              {/* Refresh */}
              <Button isIconOnly color="primary" size="sm" variant="ghost">
                <FiRefreshCw />
              </Button>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid gap-6">
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {SENSOR_CATEGORIES[
                selectedCategory as keyof typeof SENSOR_CATEGORIES
              ]?.sensors.map((sensor, index) => {
                const isSelected = selectedSensors.includes(sensor);
                const color =
                  CHART_COLORS[isDark ? "dark" : "light"][
                    index % CHART_COLORS[isDark ? "dark" : "light"].length
                  ];
