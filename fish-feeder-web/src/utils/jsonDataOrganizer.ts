...
export class JsonDataOrganizer {
  ...
  static organizeSensorData(rawData: any): SensorDataGroup {
    ...
    return {
      temperature: {
        feeder:
          rawData?.feedTemp ||
          rawData?.DHT22_FEEDER?.temperature?.value ||
          null,
        system:
          rawData?.boxTemp || rawData?.DHT22_SYSTEM?.temperature?.value || null,
        ambient: rawData?.ambientTemp || null,
      },
      humidity: {
        feeder:
          rawData?.feedHumidity ||
          rawData?.DHT22_FEEDER?.humidity?.value ||
          null,
        system:
          rawData?.boxHumidity ||
          rawData?.DHT22_SYSTEM?.humidity?.value ||
          null,
      ...
      electrical: {
        batteryVoltage:
          ...
          rawData?.loadVoltage ||
          rawData?.BATTERY_STATUS?.voltage?.value ||
          null,
        batteryPercentage: this.calculateBatteryPercentage(
          rawData?.batteryVoltage || rawData?.loadVoltage,
        ),
        loadVoltage: rawData?.loadVoltage || rawData?.batteryVoltage || null,
        loadCurrent:
          rawData?.loadCurrent ||
          rawData?.batteryCurrent ||
          rawData?.BATTERY_STATUS?.current?.value ||
          null,
        solarVoltage:
          rawData?.solarVoltage ||
          rawData?.SOLAR_VOLTAGE?.voltage?.value ||
          null,
        solarCurrent:
          rawData?.solarCurrent ||
          rawData?.SOLAR_CURRENT?.current?.value ||
          null,
      },
      mechanical: {
        feederWeight:
          rawData?.weight || rawData?.HX711_FEEDER?.weight?.value || null,
        soilMoisture:
          rawData?.soilMoisture ||
          rawData?.SOIL_MOISTURE?.moisture?.value ||
          null,
  ...
  static organizeControlData(rawData: any): ControlDataGroup {
    ...
    return {
      ...
      actuator: {
        position:
          rawData?.actuator?.position || rawData?.actuatorCommand || "stop",
  ...
  private static calculateBatteryPercentage(
    voltage: number | null,
  ): number | null {
  ...
  private static assessDataQuality(
    ...
    const hasElectrical =
      data.batteryVoltage !== null || data.loadVoltage !== null;
    const hasTimestamp =
      data.timestamp &&
